...
class Application {
    ...
    protected function getIssuesByRepoAndMilestone(string $repositoryName,int $milestoneNumber): ?array
    {
        $params = [
            'state'=>'all',
        ...
        $issuesByRepo = $this->github->api('issues')->all(Utilities::env('GH_ACCOUNT'),$repositoryName,$params);

        return( ! empty($issuesByRepo))?$issuesByRepo:null;
    }

    protected function getMilestonesByRepo(string $repositoryName): ?array
    {
        $ms=[];
